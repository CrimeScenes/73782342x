...
local function getDynamicFov(targetPosition)
    ...
    if distance <= getgenv().Ranges.CloseDistance then
        return getgenv().AdvancedTargetingSettings.CloseRangeThreshold
    -- Check if the target is within the Mid range (50 to 100)
    elseif distance <= getgenv().Ranges.MidDistance then
        return getgenv().AdvancedTargetingSettings.MidRangeThreshold
    -- Check if the target is within the Far range (100 to 200)
    elseif distance <= getgenv().Ranges.FarDistance then
        return getgenv().AdvancedTargetingSettings.FarRangeThreshold
    else
        return getgenv().AdvancedTargetingSettings.FarRangeThreshold  -- Default to Far range if the target is beyond 200 units
    end
end
